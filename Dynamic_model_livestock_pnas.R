...
#Import parameter ranges and initial values#
...
#Mean density-dependent reduction in fecundity per mated pair- exponential relationship, proportional to mean worms. 
...
MWB.dyn <- function(t, var, parameters){
  with(as.list(c(var,parameters)),{
    ...
    #3. ### FOI ON SNAILS/SHEDDING PARAMETERS  ####
    ...
    mira_S<- lambda_S*N_S*Mean_couples_S*dd_exp_i(parameters,var,Mean_F_i = Mean_F_S_Sb,b_i=b_S,k_i=k_G)
    
    mira_tot<- mira_C+mira_G+mira_S
    
    
    
    #### death rate of worms overall including tx
...
MWB.sol <- lsoda(MWB.init[10,], times, MWB.dyn, MWB.par[1,]) ### inits defined with parameters earlier
